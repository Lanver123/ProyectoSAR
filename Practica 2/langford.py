...
def langford_directo(N, allsolutions):
    ...
    def backtracking(num):
        if num <= 0:
            # Devolver resultado como 4-1-3-1-2-4-3-2
            yield "-".join(map(str, seq))
        else:
            for i in range(N*2-1-num): # Para todas las posiciones iniciales posibles dado num
                if seq[i] == 0 and seq[i+num+1] == 0: # Si se pueden colocar
                    seq[i] = num    # Ponemos los numeros
                    seq[i+num+1] = num  # Ponemos los numeros
                    for s in backtracking(num-1):   # Si con esta combinacion se encuentra una solucion
                        yield s
                    seq[i] = 0  # Backtracking
                    seq[i+num+1] = 0 # Backtracking
...
def langford_data_structure(N):
    ...
    for n in range(1, N+1):  # Para cada valor de 1 a N
        for p in range(2*N-n-1):
            Y[value(n)+position(p)] = [value(n),
                                       position(p), position(p+(n+1))]
